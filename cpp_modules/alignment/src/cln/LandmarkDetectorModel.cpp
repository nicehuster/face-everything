...
CLNF::CLNF()
{
	...
	if(parameters.curr_face_detector == FaceModelParameters::SEETA_DETECTOR)
	{
		this->face_detector = make_shared<seeta::fd::SeetaDetector>();
	}
	this->Read(parameters.model_location);
}

CLNF::CLNF(const string& model_file)
{
    this->Read(model_file);
}
// Constructor from a model file
CLNF::CLNF(const FaceModelParameters& param)
{
	if(param.curr_face_detector == FaceModelParameters::SEETA_DETECTOR)
	{
		this->face_detector = make_shared<seeta::fd::SeetaDetector>();
	}
	this->Read(param.model_location);
}
...
//	{
//		this->face_detector->load_model({},{face_detector_location});
//	}
...
//		}
		...
		// Copy over the hierarchical example
...
bool CLNF::init_tracker(const cv::Mat_<uchar>& image,
...
bool CLNF::update_tracker(const cv::Mat_<uchar> &image,
                          const vector<cv::Point2f>&landmarks,
                          FaceModelParameters& params){

    //actual don't need this one
//    Fit(image, params.window_sizes_current, params);
cout <<"d1" << endl; 
    const int N = pdm.NumberOfPoints();
	...
	for(int i = 0; i < n; i++)
	{
		...
		for(int ii = 0; ii < resp_size; ii++)
		{
			for(int jj = 0; jj < resp_size; jj++)
			{
				...
				// the KDE test of that point multiplied by the probability at the current, xi, yi
				float v = (*p++) * (*kde_it++);
...
cv::Rect_<double> CLNF::get_bbox(float scale, float shift_scale) const
{
    cv::Mat_<double> xs = this->detected_landmarks(cv::Rect(0,0,1,this->detected_landmarks.rows/2));
    cv::Mat_<double> ys = this->detected_landmarks(cv::Rect(0,this->detected_landmarks.rows/2, 1, this->detected_landmarks.rows/2));
    ...
    cv::Rect_<double> face_rect = {
            std::max<double>(0, min_x - scale*width/2),
            std::max<double>(0, min_y - (scale + shift_scale)*width/2),
            (1 + scale)*width,
            (1 + scale)*width,
    };
...
cv::Mat CLNF::get_aligned_face(const cv::Mat& src) const {
