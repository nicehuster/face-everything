...
void matchTemplate_m(  const cv::Mat_<float>& input_img, cv::Mat_<double>& img_dft, cv::Mat& _integral_img, cv::Mat& _integral_img_sq, const cv::Mat_<float>&  templ, map<int, cv::Mat_<double> >& templ_dfts, cv::Mat_<float>& result, int method )
{
	...
	// Assume temp_result is defined properly
	if(result.empty())
	{
...
//    int mouth_start_id = 48;
	// Drawing feature points
	if(n >= 66)
	{
		for( int i = 0; i < n; ++i)
//		for(int i = 0; i < cl::fa::five_landmark_idx.size(); ++i)
		{
//			int idx = cl::fa::five_landmark_idx[i];
			int idx = i;
			if(visibilities.at<int>(idx))
			{
...
bool DetectFaces(vector<cv::Rect_<double> >& o_regions, const cv::Mat_<uchar>& intensity, cl::fd::Detector* detector)
{
	...
	std::vector<cl::FaceBox> detect_results;
	cv::Mat intensity_copy = intensity;
	if(!detector->detect(intensity_copy, detect_results))
		return false;

	for(auto& e: detect_results)
	{
		face_detections.push_back(e.bbox_);
	}
...
bool DetectSingleFace(cv::Rect_<double>& o_region, const cv::Mat_<uchar>& intensity_image,  cl::fd::Detector* detector, cv::Point preference)
{
	// The tracker can return multiple faces
	vector<cv::Rect_<double> > face_detections;
	bool detect_success = LandmarkDetector::DetectFaces(face_detections, intensity_image, detector);
