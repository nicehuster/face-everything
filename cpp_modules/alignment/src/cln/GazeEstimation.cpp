...
void FaceAnalysis::EstimateGaze(const LandmarkDetector::CLNF& clnf_model, cv::Point3f& gaze_absolute, float fx, float fy, float cx, float cy, bool left_eye)
{
	...
	cv::Point3f rayDir = cv::Point3f(pupil.x/ norm(pupil),pupil.y/ norm(pupil),pupil.z/ norm(pupil));
	...
	gaze_absolute = cv::Point3f(gazeVecAxis.x/ norm(gazeVecAxis),gazeVecAxis.y/ norm(gazeVecAxis),gazeVecAxis.z/ norm(gazeVecAxis));
}
...
void FaceAnalysis::DrawGaze(cv::Mat img, const LandmarkDetector::CLNF& clnf_model, cv::Point3f gazeVecAxisLeft, cv::Point3f gazeVecAxisRight, float fx, float fy, float cx, float cy)
{
	...
	points_left.push_back(cv::Point3f(pupil_left + gazeVecAxisLeft*50.0));

	vector<cv::Point3f> points_right;
	points_right.push_back(cv::Point3f(pupil_right));
	points_right.push_back(cv::Point3f(pupil_right + gazeVecAxisRight*50.0));
