...
int main(int argc, char** argv) {
    ...
    const string cln_model_dir =  "../trained_models/alignment/cln";
    const string mtcnn_model_dir = "../trained_models/detection";
    const string openpose_model_dir = "../trained_models/alignment/op_face";
    const string recognition_model_dir = "../trained_models/recognition";
    const string facedb_dir = "../data/facedb";
    const string video_dir = "../data";
    ...
    VideoCapture cap;
    if(!cap.open(video_dir+"/test.mp4")){
        cap.open(0);
    }
    if(!cap.isOpened())
    {
...
#pragma omp parallel for num_threads(CL_NUM_THREADS)
            ...
            for(int model = 0; model < clnf_models.size(); ++model)
            {
                if (tracking_info[model].tracking_){
                    ...
                    cv::Mat temp=src_display(detect_mask).clone();
                    cv::putText(temp,
                                full_msg,
                                position,
